...
class Rule {
  ...
  freq(dice) {
    ...
    */
    ...
    // freqs.values() makes {2, 1, 1, 1}
    // Array.from makes from {2, 1, 1, 1} =>  array  [2, 1, 1, 1]
  }
...
class FullHouse extends Rule {
  evalRoll = dice => {
    ...
    // freq return array of qty of repeating values. For fullHouse we need array [2,3]
    // and we check for it
    return d.size >= 4 && ((!d.has(1) || !d.has(6))
                            || !d.has(1,2) 
                            || !d.has(5,6) 
                            || !d.has(1,6)) ? this.score : 0;
  }
}
